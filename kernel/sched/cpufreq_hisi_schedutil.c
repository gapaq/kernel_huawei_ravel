...
#include "sched.h"
#include "tune.h"
#ifdef CONFIG_HISI_CPU_FREQ_GOV_HISI_SCHEDUTIL
#include "walt.h"
#include <linux/hisi_rtg.h>

#define CREATE_TRACE_POINTS
#include <trace/events/cpufreq_hisi_schedutil.h>
#endif
...
#endif
	trace_cpufreq_hisi_schedutil_eval_target(sg_policy->max_cpu,
					    util, max, cpu_load,
					    policy->cur, new_freq);
...
#ifndef CONFIG_HISI_CPU_FREQ_GOV_HISI_SCHEDUTIL
 ...
 * @sg_policy: hisi_schedutil policy object to compute the new frequency for.
		...
		u64 time, unsigned int *early_detection)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		trace_cpufreq_hisi_schedutil_get_util(j, j_sg_cpu->util,
						 j_sg_cpu->max,
				...
				unsigned int flags)
{
	...
	if (!raw_spin_trylock(&sg_policy->update_lock)) {
		trace_cpufreq_hisi_schedutil_notyet(sg_cpu->cpu,
...
static bool sugov_time_limit(struct sugov_policy *sg_policy, unsigned int next_freq)
{
	...
	if (!skip_hispeed_logic &&
		...
		if (delta_ns < NSEC_PER_USEC *
			...
			trace_cpufreq_hisi_schedutil_notyet(sg_policy->max_cpu,
...
#endif /* CONFIG_HISI_CPU_FREQ_GOV_HISI_SCHEDUTIL */
...
//		sched_update_group_load(rq);
	...
	if (next_freq == sg_policy->next_freq) {
		trace_cpufreq_hisi_schedutil_already(sg_policy->max_cpu,
						sg_policy->next_freq, next_freq);
		goto out;
	}
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the hisi_schedutil governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the hisi_schedutil governor rejects all other frequency scaling requests.
	 *
		...
		const char *buf, size_t count)
{
	...
	if (tunables->boost) {
		trace_cpufreq_hisi_schedutil_boost("on");
		if (!tunables->boosted)
			sugov_boost(attr_set);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_hisi_schedutil_unboost("off");
	}
		...
		const char *buf, size_t count)
{
	...
	trace_cpufreq_hisi_schedutil_boost("pulse");
...
static struct governor_user_attr hisi_schedutil_user_attrs[] = {
#ifdef CONFIG_HISI_CPU_FREQ_GOV_HISI_SCHEDUTIL
...
static struct cpufreq_governor hisi_schedutil_gov;
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	ret = kobject_init_and_add(&tunables->attr_set.kobj, &sugov_tunables_ktype,
				   ...
				   hisi_schedutil_gov.name);
	if (ret)
		goto fail;

	gov_sysfs_set_attr(policy->cpu, hisi_schedutil_gov.name, hisi_schedutil_user_attrs);
out:
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_HISI_SCHEDUTIL
struct cpufreq_governor *cpufreq_default_governor(void)
{
	return &hisi_schedutil_gov;
}
#endif

static int __init sugov_register(void)
{
	return cpufreq_register_governor(&hisi_schedutil_gov);
}
