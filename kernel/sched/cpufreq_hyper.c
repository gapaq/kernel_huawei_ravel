...
#define UP_RATE_LIMIT				10
#define DOWN_RATE_LIMIT				50
#define BIT_SHIFT_1 				8
#define BIT_SHIFT_2 				7
#define TARGET_LOAD_1				40
#define TARGET_LOAD_2				75

#define UP_RATE_LIMIT_BIGC			10
#define DOWN_RATE_LIMIT_BIGC		50
#define BIT_SHIFT_1_BIGC 			9
#define BIT_SHIFT_2_BIGC 			7
#define TARGET_LOAD_1_BIGC 			40
#define TARGET_LOAD_2_BIGC 			75

#define hygov_KTHREAD_PRIORITY		25
...
static DEFINE_PER_CPU(struct hygov_tunables, cached_tunables);
...
static bool hygov_up_down_rate_limit(struct hygov_policy *sg_policy, u64 time,
				     unsigned int next_freq)
{
	s64 delta_ns;
				...
				unsigned int next_freq)
{
	...
	if (hygov_up_down_rate_limit(sg_policy, time, next_freq))
		return;
 ...
 * @sg_cpu: hyper cpu object to compute the new frequency for.
 ...
 */
static unsigned int get_next_freq(struct hygov_cpu *sg_cpu, unsigned long util,
				  unsigned long max)
{
...
static void hygov_get_util(unsigned long *util, unsigned long *max, u64 time)
{
...
static void hygov_iowait_boost(struct hygov_cpu *sg_cpu, unsigned long *util,
			       unsigned long *max)
{
...
static void hygov_update_single(struct update_util_data *hook, u64 time,
				unsigned int flags)
{
	struct hygov_cpu *sg_cpu = container_of(hook, struct hygov_cpu, update_util);
	struct hygov_policy *sg_policy = sg_cpu->sg_policy;
	struct cpufreq_policy *policy = sg_policy->policy;
	...
	if (flags & SCHED_CPUFREQ_DL) {
		...
		hygov_get_util(&util, &max, time);
		hygov_iowait_boost(sg_cpu, &util, &max);
...
static unsigned int hygov_next_freq_shared(struct hygov_cpu *sg_cpu,
					   ...
					   unsigned int flags)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		hygov_iowait_boost(j_sg_cpu, &util, &max);
				...
				unsigned int flags)
{
	...
	if (hygov_should_update_freq(sg_policy, time)) {
		next_f = hygov_next_freq_shared(sg_cpu, util, max, flags);
		hygov_update_commit(sg_policy, time, next_f);
	}
...
{
	struct hygov_policy *sg_policy = container_of(work, struct hygov_policy, work);
...
static void hygov_irq_work(struct irq_work *irq_work)
{
	...
	sg_policy = container_of(irq_work, struct hygov_policy, irq_work);

	/*
	 * For Real Time and Deadline tasks, hyper governor shoots the
	 * frequency to maximum. And special care must be taken to ensure that
	 * this kthread doesn't result in that.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the hygov_work() and before that hyper
...
static void update_min_rate_limit_us(struct hygov_policy *sg_policy)
{
...
static int hygov_kthread_create(struct hygov_policy *sg_policy)
{
	...
	init_kthread_work(&sg_policy->work, hygov_work);
	init_kthread_worker(&sg_policy->worker);
	thread = kthread_create(kthread_worker_fn, &sg_policy->worker,
				"hygov:%d",
				cpumask_first(policy->related_cpus));
	if (IS_ERR(thread)) {
		pr_err("failed to create hygov thread: %ld\n", PTR_ERR(thread));
...
static void hygov_tunables_free(struct hygov_tunables *tunables)
{
	if (!have_governor_per_policy())
...
static void store_tunables_data(struct hygov_tunables *tunables,
		struct cpufreq_policy *policy)
{
	struct hygov_tunables *ptunables;
...
static void get_tunables_data(struct hygov_tunables *tunables,
		struct cpufreq_policy *policy)
{
	struct hygov_tunables *ptunables;
	unsigned int lat;
...
static int hygov_init(struct cpufreq_policy *policy)
{
	...
	ret = kobject_init_and_add(&tunables->attr_set.kobj, &hygov_tunables_ktype,
				   get_governor_parent_kobj(policy), "%s",
				   cpufreq_gov_hyper.name);
	if (ret)
		goto fail;
...
static int hygov_exit(struct cpufreq_policy *policy)
{
...
static int hygov_stop(struct cpufreq_policy *policy)
{
	struct hygov_policy *sg_policy = policy->governor_data;
	unsigned int cpu;
...
static int hygov_limits(struct cpufreq_policy *policy)
{
	struct hygov_policy *sg_policy = policy->governor_data;
...
static int cpufreq_hyper_cb(struct cpufreq_policy *policy,
				unsigned int event)
{
	switch(event) {
	case CPUFREQ_GOV_POLICY_INIT:
		return hygov_init(policy);
	case CPUFREQ_GOV_POLICY_EXIT:
		return hygov_exit(policy);
	case CPUFREQ_GOV_START:
		return hygov_start(policy);
	case CPUFREQ_GOV_STOP:
		return hygov_stop(policy);
	case CPUFREQ_GOV_LIMITS:
		return hygov_limits(policy);
	default:
		BUG();
	}
}
