 ...
 * @sg_policy: legend policy object to compute the new frequency for.
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the legend governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the legend governor rejects all other frequency scaling requests.
	 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_LEGEND
static
#endif
struct cpufreq_governor cpufreq_gov_legend;
...
static int cpufreq_legend_cb(struct cpufreq_policy *policy,
				unsigned int event)
{
	switch(event) {
...
struct cpufreq_governor cpufreq_gov_legend = {
	.name = "legend",
	.governor = cpufreq_legend_cb,
	.owner = THIS_MODULE,
};

static int __init sugov_register(void)
{
	return cpufreq_register_governor(&cpufreq_gov_legend);
}
