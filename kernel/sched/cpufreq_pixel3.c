 ...
 * @sg_policy: pixel3 policy object to compute the new frequency for.
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the pixel3 governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the pixel3 governor rejects all other frequency scaling requests.
	 *
...
static struct cpufreq_governor cpufreq_gov_pixel3;
...
static int cpufreq_pixel3_cb(struct cpufreq_policy *policy,
				unsigned int event)
{
	switch(event) {
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_PIXEL3
static
#endif
struct cpufreq_governor cpufreq_gov_pixel3 = {
	.name = "pixel3",
	.governor = cpufreq_pixel3_cb,
	.owner = THIS_MODULE,
};

static int __init sugov_register(void)
{
	return cpufreq_register_governor(&cpufreq_gov_pixel3);
}
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_PIXEL3
struct cpufreq_governor *cpufreq_default_governor(void) {
	return &cpufreq_gov_pixel3;
}
#endif
